...
def _write_readme(scenarios):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("- `figures/` 每个工况一张状态+输出组合图 `caseN_*_states.png`（6 状态反馈量 + Tp/T；"
                "工况 2/3 目标叠进对应变量面板，工况 4 追加腿长跟踪两格为 10 图）+ `summary_headline.png`\n")
...
def main(argv=None):
    ...
    for s in scenarios:
        ...
        # 每工况一张状态+输出组合图：6 状态反馈量 + 2 路控制输出(Tp/T)；
        # 工况 2/3 把目标叠进对应变量面板，工况 4 追加腿长跟踪两格扩为 10 图
        sp = plotting.plot_states(s, runs, config.FIG_DIR)
        print(f"       → 状态+输出组合图 {os.path.relpath(sp, config.REPO_ROOT)}")
